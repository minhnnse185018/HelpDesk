...
function StaffSubTickets() {
  ...
  const loadSubTickets = async () => {
    try {
      setLoading(true)
      setError('')
      const response = await apiClient.get('/api/v1/sub-tickets/assigned-to-me')
  ...
  const handleDeny = async (subTicketId, reason) => {
    try {
      await apiClient.post(`/api/v1/sub-tickets/${subTicketId}/deny`, { reason })
      setDenyModal(null)
      await loadSubTickets()
    } catch (err) {
  ...
  const handleResolve = async (subTicketId, resolutionNote) => {
    try {
      await apiClient.patch(`/api/v1/sub-tickets/${subTicketId}/resolve`, { resolutionNote })
  ...
  return (
    <div className="page">
      ...
      {!loading && !error && subTickets.length === 0 && (
        <div className="card" style={{ padding: '2rem', textAlign: 'center' }}>
          <p style={{ color: '#6b7280' }}>No sub-tickets assigned to you yet.</p>
        </div>
      )}
