    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f7' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 600, color: '#111827', marginBottom: '0.5rem', margin: 0, letterSpacing: '-0.02em' }}>
            Ticket Management
          </h1>
...
function AllTicketsTab() {
  ...
  const handleAssign = async (ticketId, staffId, priority) => {
    try {
      ...
      setAssignModal(null)
      setNotification({ type: 'success', message: 'Ticket assigned to staff successfully!' })
      await loadTickets()
    } catch (err) {
      ...
      <div style={{ 
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ 
            ...
            <thead>
              <tr style={{ backgroundColor: 'rgba(249, 250, 251, 0.5)', borderBottom: '1px solid rgba(229,231,235,0.6)' }}>
                ...
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Categories</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Assignee</th>
              ...
              {tickets.length === 0 ? (
                <tr>
                  <td colSpan="9" style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>
                    <div style={{ fontSize: '0.875rem', fontWeight: 500 }}>No tickets found</div>
                  </td>
                </tr>
              ) : (
                tickets.map((ticket) => (
                  <tr key={ticket.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '1rem', color: '#111827', fontWeight: 500 }}>{ticket.title}</td>
                    <td style={{ padding: '1rem' }}>{getStatusBadge(ticket.status)}</td>
                    <td style={{ padding: '1rem' }}>{getPriorityBadge(ticket.priority)}</td>
                    <td style={{ padding: '1rem', color: '#6b7280' }}>{ticket.room?.name || 'N/A'}</td>
                    <td style={{ padding: '1rem', color: '#6b7280' }}>{ticket.department?.name || 'N/A'}</td>
                    <td style={{ padding: '1rem', color: '#6b7280', fontSize: '0.8rem' }}>
                      {Array.isArray(ticket.ticketCategories) && ticket.ticketCategories.length > 0
                        ? ticket.ticketCategories.map(tc => tc.category?.name).filter(Boolean).join(', ')
                        : 'N/A'}
                    </td>
                    <td style={{ padding: '1rem', color: '#6b7280' }}>{ticket.assignee?.username || ticket.assignee?.fullName || 'N/A'}</td>
                    <td style={{ padding: '1rem', color: '#6b7280', fontSize: '0.8rem' }}>{formatDate(ticket.createdAt)}</td>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                        {(() => {
                          const hasNoAssignee = !ticket.assignee && !ticket.assignedTo
                          const hasOneCategory = ticket.ticketCategories?.length === 1
                          const canAssign = hasNoAssignee && hasOneCategory
                          ...
                          return canAssign ? (
                            <button
                              ...
                              onMouseOver={(e) => { e.target.style.backgroundColor = 'rgba(255, 255, 255, 0.15)'; e.target.style.transform = 'translateY(-2px)'; e.target.style.boxShadow = '0 12px 40px rgba(0, 0, 0, 0.15), inset 0 1px 0 rgba(255, 255, 255, 0.7)'; }}
                              onMouseOut={(e) => { e.target.style.backgroundColor = 'rgba(255, 255, 255, 0.08)'; e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 8px 32px rgba(0, 0, 0, 0.12), inset 0 1px 0 rgba(255, 255, 255, 0.6), inset 0 -1px 0 rgba(0, 0, 0, 0.05)'; }}
                            >
                              Assign
                            </button>
                          ) : null
                        ...
                        <button
                          ...
                          onMouseOver={(e) => { e.target.style.backgroundColor = 'rgba(239, 68, 68, 0.15)'; e.target.style.transform = 'translateY(-2px)'; e.target.style.boxShadow = '0 12px 40px rgba(239, 68, 68, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.5)'; }}
                          onMouseOut={(e) => { e.target.style.backgroundColor = 'rgba(239, 68, 68, 0.08)'; e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 8px 32px rgba(239, 68, 68, 0.15), inset 0 1px 0 rgba(255, 255, 255, 0.4), inset 0 -1px 0 rgba(239, 68, 68, 0.1)'; }}
                        >
                          Delete
                        </button>
                      </div>
...
function PendingSplitTab() {
  ...
  const handleSplit = async (ticketId, groups) => {
    try {
      await apiClient.post(`/api/v1/tickets/${ticketId}/split-categories`, { groups })
      setSplitModal(null)
      setNotification({ type: 'success', message: 'Ticket split successfully! Sub-tickets have been created.' })
      await loadTickets()
    } catch (err) {
      ...
      <div style={{ 
        ...
        <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(243,244,246,0.6)' }}>
          <p style={{ margin: 0, color: '#6b7280', fontSize: '0.875rem' }}>
            Tickets with multiple categories that need to be split into sub-tickets
          </p>
        </div>
        
        {tickets.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>
            <div style={{ fontSize: '0.875rem', fontWeight: 500 }}>No tickets pending split</div>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ 
...
function SplitCategoriesModal({ ticket, onClose, onSubmit }) {
  ...
  useEffect(() => {
    const loadStaff = async () => {
      try {
        ...
        let data = response?.data || response
        if (data && !Array.isArray(data)) data = data.users || data.data || []
  ...
  const handleSubmit = async () => {
    const groups = ticket.categories?.map((cat) => ({
      categoryIds: [cat.id],
      ...(groupAssignments[cat.id] && { staffId: groupAssignments[cat.id] }),
    })) || []

    setSubmitting(true)
    await onSubmit(ticket.id, groups)
    setSubmitting(false)
  }
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ 
            ...
            <p style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem', color: '#6b7280' }}>
              <strong style={{ color: '#374151' }}>Ticket:</strong> {ticket.title}
            </p>
          ...
          <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            >
              <div style={{ marginBottom: '0.25rem' }}>
                <span style={{ fontWeight: 600, color: '#374151' }}>Room: </span>
                <span style={{ color: '#6b7280' }}>{ticket.room?.name || 'N/A'}</span>
              </div>
              <div style={{ marginBottom: '0.25rem' }}>
                <span style={{ fontWeight: 600, color: '#374151' }}>Current Priority: </span>
...
function WaitingAcceptanceTab() {
  ...
  return (
    <div style={{ 
      ...
      {tickets.length === 0 ? (
        <div style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>
          <div style={{ fontSize: '0.875rem', fontWeight: 500 }}>No tickets waiting for acceptance</div>
        </div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ 
            ...
            <thead>
              <tr style={{ backgroundColor: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                ...
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Priority</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Room</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Assigned To</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Created At</th>
              </tr>
            </thead>
            <tbody>
...
function OverdueTicketsTab() {
  ...
  return (
    <div style={{ 
      ...
      {tickets.length === 0 ? (
        <div style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>
          <div style={{ fontSize: '0.875rem', fontWeight: 500 }}>No overdue tickets</div>
        </div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ 
            ...
            <thead>
              <tr style={{ backgroundColor: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                ...
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Room</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Assignee</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Due Date</th>
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Created At</th>
              </tr>
            </thead>
            <tbody>
              {tickets.map((ticket) => (
                <tr key={ticket.id} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: '#fef2f2' }}>
                  <td style={{ padding: '1rem', color: '#111827', fontWeight: 500 }}>{ticket.title}</td>
                  <td style={{ padding: '1rem' }}>{getStatusBadge(ticket.status)}</td>
                  <td style={{ padding: '1rem' }}>{getPriorityBadge(ticket.priority)}</td>
                  <td style={{ padding: '1rem', color: '#6b7280' }}>{ticket.room?.name || 'N/A'}</td>
                  <td style={{ padding: '1rem', color: '#6b7280' }}>{ticket.assignee?.username || ticket.assignee?.fullName || 'N/A'}</td>
                  <td style={{ padding: '1rem', color: '#dc2626', fontSize: '0.8rem', fontWeight: 500 }}>{formatDate(ticket.dueDate)}</td>
                  <td style={{ padding: '1rem', color: '#6b7280', fontSize: '0.8rem' }}>{formatDate(ticket.createdAt)}</td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div
      style={{
        ...
        right: '2rem',
        zIndex: 9999,
        backgroundColor: type === 'success' ? 'rgba(220, 252, 231, 0.25)' : 'rgba(254, 226, 226, 0.25)',
        border: `1px solid ${type === 'success' ? 'rgba(134, 239, 172, 0.3)' : 'rgba(254, 202, 202, 0.3)'}`,
        borderRadius: '16px',
        padding: '1rem 1.5rem',
